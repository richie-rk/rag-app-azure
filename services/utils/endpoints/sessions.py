...
Consolidates Max AI's 3 save functions + 2 retrieve functions into
clean single save/retrieve/delete using Azure Table Storage.
"""
...
def save_session(data: dict) -> dict:
    """Save or update a chat session turn in Azure Table Storage.

    Uses the upsert pattern from Max AI's save_feedback (the best save function).
    """
...
def get_sessions(username: str, session_id: str | None = None) -> list[dict]:
    """Retrieve chat sessions for a user, optionally filtered by session_id.

    Consolidates Max AI's retrieve_chat_history + retrieve_session_history.
    """
    table_client = _get_table_client()
